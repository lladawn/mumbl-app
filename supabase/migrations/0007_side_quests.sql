create table if not exists side_quest_cards (
...
create index if not exists side_quest_cards_active_idx
  on side_quest_cards (space_id, status, expires_at desc);
create index if not exists side_quest_cards_owner_idx
  on side_quest_cards (space_id, owner_session_token_hash, status);

create table if not exists side_quest_rooms (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists side_quest_rooms_session_idx
...
create table if not exists side_quest_messages (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists side_quest_messages_room_created_idx
  on side_quest_messages (room_id, created_at);
create index if not exists side_quest_messages_expiry_idx
  on side_quest_messages (expires_at);

create table if not exists side_quest_reports (
  id uuid primary key default gen_random_uuid(),
  room_id uuid not null references side_quest_rooms(id) on delete cascade,
